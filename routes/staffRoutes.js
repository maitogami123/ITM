...
router.get('/export/salary', authMiddleware(['superadmin', 'leader', 'lecturer']), exportSalaryIncrementsToExcel);

// Delete a staff member by ID (requires superadmin role)
router.delete('/:id', authMiddleware(['superadmin']), deleteStaff);

router.patch('/:id/teacher-grade', authMiddleware(['superadmin']), updateTeacherGrade);

router.patch('/:id/promote', authMiddleware(['superadmin']), promoteSalaryLevel);
